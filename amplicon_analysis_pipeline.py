...
def check_errors():
    ...
    with open('Amplicon_analysis_pipeline.log','rt') as pipeline_log:
...
""")
    ...
    with open('pipeline.log','rt') as pipeline_log:
        log = pipeline_log.read()
        if "Errors and/or warnings detected in mapping file" in log:
            with open("Metatable_log/Metatable.log","rt") as metatable_log:
...
if __name__ == "__main__":
    ...
    with open("Final_name.txt",'wt') as final_name:
    ...
    # Run the pipeline
    with open("pipeline.log","wt") as pipeline_out:
        try:
    ...
    if os.path.exists(log_file):
        ...
        if exit_code == 0:
            ...
            with open("pipeline_outputs.html","wt") as html_out:
                html_out.write("""<html>
<head>
...
""")
            ...
            with open("pipeline.log",'rt') as log:
    ...
    with open("fastqc_quality_boxplots.html","wt") as quality_boxplots:
...
""")
    ...
    if args.pipeline == "DADA2":
        ...
        with open(error_rate_plots_html,"wt") as error_rate_plots_out:
...
""")
    ...
    if args.categories_file is not None:
        ...
        with open("alpha_diversity_boxplots.html","wt") as boxplots_out:
